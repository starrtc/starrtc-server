	...
	try{
		...
		if($pstmt->execute(array($userId))){ 
			...
			if(!empty($groupList)){
				...
				foreach ($pureGroupArr as $groupId){
					...
					if($pstmt->execute(array($groupId))){
						...
						if($resNum == 1){	
							...
							$groupNameArr[$index] = $result[0][0];
							$creatorArr[$index] = $result[0][1];			
							$index++;
						}else{
				...
				$data = array();
				if(!empty($groupIdArr)){
					$data['groupIdList'] 		= implode(',', $groupIdArr);
				}	
				if(!empty($groupNameArr)){
					$data['groupNameList'] 		= implode(',', $groupNameArr);
				}				
				if(!empty($creatorArr)){
